...
export default function EditArticlePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchData = async () => {
      try {
        ...
        if (article) {
          ...
          setIsOSINT(article.isOSINT || false);
        } else {
  ...
  if (loading) {
    return <div className="flex justify-center p-12 text-gray-500"><Loader2 className="animate-spin" /></div>;
  }
    ...
    <div className="max-w-6xl">
      <div className="flex justify-between items-center mb-8 border-b border-[#2E2E2E] pb-4">
        <h1 className="text-3xl font-black uppercase tracking-widest text-white flex items-center">
          <FileText size={28} className="mr-3 text-[#FF6B00]" /> Edit Intelligence
        </h1>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          >
            {saving ? <Loader2 size={24} className="animate-spin" /> : <><Save size={24} className="mr-2" /> Update Intelligence</>}
          </Button>
        </div>
      </form>
    </div>
